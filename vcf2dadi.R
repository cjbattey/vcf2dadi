...
#            and second listing population assignment as an integer (same format as STACKS popmap files). 
#            Any sep should work. pop1/pop2/pop3 will be in order of these integer values, so I suggest 1/2/3. 
#            IDK what will happen if you use character strings for population names. 
#
#optional parameters:
#outgroup=text string giving unique sample ID of a single outgroup sample (default NULL)
...
#minsamples=beta version - not recommended. numeric vector of minimum individuals per population to retain a locus. Currently only works for 2 populations. 
...
#biallelic=remove non-biallelic loci? (default False) NOTE: This breaks on stacks vcf's(!)
#cores=number of processing cores to use (default 1)
#oneSNPperLocus=sample a single SNP per locus? (default F). NOTE: bootstraps are over SNP's, so only independent if sampling 1 SNP per locus.
...
vcf2dadi <- function(vcf,populations,allele.freq=F,outgroup=NULL,min.maf=NULL,outfile=NULL,minsamples=NULL,
                     bootstrap=F,n.bootstraps=NULL,bootstrap.dir=NULL,cores=1,oneSNPperLocus=F){
  ...
  #read in data
  vcf <- data.frame(fread(vcf,header=T))
  nSNP <- nrow(vcf)
  nLoci <- nlevels(factor(vcf$X.CHROM))
  
  #remove sites w>2 alleles (temp disabled bc it breaks on stacks datasets)
  # if(biallelic==T){
  ...
  if (nPop==1){
    ...
    colnames(dadiOut) <- c("IngroupSeq","OutgroupSeq","Allele1","pop1","Allele2","pop1","Locus","Site")
  } else if (nPop==2){
    dadiOut <- foreach(i=1:nrow(vcf),.combine = rbind) %dopar% makeDadiRow_k2(i,vcf,outgroup,outcol,populations,min.maf,allele.freq)
    colnames(dadiOut) <- c("IngroupSeq","OutgroupSeq","Allele1","pop1","pop2","Allele2","pop1","pop2","Locus","Site")
  } else if (nPop==3){
    dadiOut <- foreach(i=1:nrow(vcf),.combine = rbind) %dopar% makeDadiRow_k3(i,vcf,outgroup,outcol,populations,min.maf,allele.freq)
    colnames(dadiOut) <- c("IngroupSeq","OutgroupSeq","Allele1","pop1","pop2","pop3","Allele2","pop1","pop2","pop3","Locus","Site")
